...
const PlatformModal = ({ platform, onClose, onSuccess, forAdmin = false }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    apiKey: platform?.apiKey || "", // ✅ Include API key in form data
  });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white w-full max-w-3xl p-6 rounded-lg shadow-lg overflow-y-auto max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Preference Weight + Parser Type + IsActive */}
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex flex-col flex-1">
              ...
              <input
                ...
                className={`border rounded p-2 ${errors.preferenceWeight ? "border-red-500" : ""}`}
              />
          ...
          {/* API Key section */}
          {formData.parserType === "API" && (
            <div className="flex flex-col relative">
              <label className="font-medium mb-1">API Key</label>
              ...
              {(!formData.apiKey || formData.apiKey.trim() === "") && (
                <p className="text-gray-500 text-sm mt-1">No API key provided — this platform may not work properly until configured.</p>
              )}
            </div>
          )}
