...
const UserJobBoard = () => {
    ...
    const fetchJobs = async (page = 1, keywordInput = "", where = "Canada", sort_by = "date") => {
        setLoading(true);
        ...
        const searchKeyword = keywordInput.trim() || "Software Engineer"; // ✅ fallback if blank

        try {
            const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/jobs`, {
                headers: { Authorization: `Bearer ${token}` },
                params: {
                    keyword: searchKeyword, // ✅ use consistent 'keyword' param
                    where,
                    sort_by,
                    page,
                    results_per_page: 20,
            ...
            const jobList = Array.isArray(res.data?.data?.results)
    ...
    const handleSearch = (e) => {
        e.preventDefault();
        setCurrentPage(1);
        fetchJobs(1, keyword, location, sortBy);
    };
    ...
    const handlePageChange = (newPage) => {
        ...
        fetchJobs(newPage, keyword, location, sortBy);
    };
            ...
            <div className="max-w-6xl mx-auto p-4 space-y-6">
                ...
                <div className="text-center space-y-2">
                    ...
                    <p className="text-sm text-gray-600 max-w-xl mx-auto">
                        Search fresh job opportunities updated in real-time.
                    </p>
                </div>
                ...
                >
                    ...
                    <button
                        type="submit"
                        className="flex items-center bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg px-4 py-2 transition shadow"
                    >
                ...
                <div className="flex justify-end">
                    <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 rounded-lg border text-gray-700 border-gray-300 hover:bg-gray-100 transition text-sm"
                    >
                ...
                <div className="flex flex-col lg:flex-row gap-6">
                        ...
                        {loading ? (
                            ...
                            jobs.map((job) => (
                                <div
                                    ...
                                    className={`p-4 cursor-pointer border-b hover:bg-gray-50 ${selectedJob?.id === job.id ? "bg-blue-50 border-l-4 border-blue-600" : ""
                                        }`}
                                >
                    ...
                    {/* Job Details */}
                    <div className="flex-1 bg-white rounded-2xl shadow border p-6 min-h-[300px]">
                        {selectedJob ? (
                            <>
                                ...
                                {selectedJob.redirect_url && (
                                    <a
                                        ...
                                        className="inline-flex items-center text-white bg-green-600 hover:bg-green-700 rounded-lg px-4 py-2 gap-2 shadow text-sm"
                                    >
                ...
                {jobs.length > 0 && (
                    <div className="flex justify-center gap-4 items-center pt-6 text-sm">
                        <button
                            ...
                            className={`flex items-center gap-1 px-4 py-2 rounded-lg border ${currentPage === 1
                        ...
                        <button
                            ...
                            onClick={() => handlePageChange(currentPage + 1)}
                            className={`flex items-center gap-1 px-4 py-2 rounded-lg border ${currentPage === totalPages
                                    ? "text-gray-400 border-gray-200 cursor-not-allowed"
                                    : "text-gray-700 border-gray-300 hover:bg-gray-100"
                                }`}
                        >
